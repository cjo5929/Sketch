...
def get_vector(data):
    ...
    text2 = []
    for w in text.split():
        if stopwords.get(w) == None:
            text2.append(w)
    text = ' '.join(text2)
    text = rhinoMorph.onlyMorph_list(rn, text, pos=['NNG', 'NNP', 'VV', 'XR', 'IC', 'MAG', 'MAJ'])
    ...
    for word in word_sort:
        try:
            vec += diary.apps.DiaryConfig.glove_vectors.get_vector(word)
        except:
            continue
        cnt += 1
        if cnt == 10:
